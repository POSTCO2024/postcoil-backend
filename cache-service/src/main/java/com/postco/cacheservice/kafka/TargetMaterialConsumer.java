...
public class TargetMaterialConsumer extends GenericKafkaConsumer<TargetMaterialDTO.View> {
    ...
    public TargetMaterialConsumer(ObjectMapper objectMapper,
                                  TargetMaterialQueryService queryService,
                                  TargetMaterialCommandService commandService) {
        super(objectMapper, queryService, commandService);
    }

    @Override
    protected TargetMaterialDTO.View deserializeMessage(String message) {
        try {
            return objectMapper.readValue(message, TargetMaterialDTO.View.class);
        } catch (JsonProcessingException e) {
            log.error("[Kafka 실패] 작업대상재 메시지 역직렬화 중 오류 발생: {}", message, e);
            return null;
        }
    }

    @Override
    protected String getDataId(TargetMaterialDTO.View data) {
